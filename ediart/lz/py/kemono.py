...
class SlideshowHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        proxies = spider.proxies if spider else None
        ...
        if path == '/gallery.html':
            try:
                ...
                images = spider.post_image_cache.get(cache_key, []) if spider else []
                if not images:
                    self.send_response(200)
                    self.end_headers()
                    self.wfile.write(b"<h2 style='color:#ccc;text-align:center;margin-top:20%'>Loading...</h2>")
                    return
                img_tags = "".join([f'<div class="image-item"><img src="{url}" loading="lazy" referrerpolicy="no-referrer"></div>' for url in images])
                html = f"""<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{{margin:0;padding:5px;background:#121212}}.image-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(100%,1fr));gap:5px}}.image-item{{background:#222;border-radius:4px;overflow:hidden}}.image-item img{{width:100%;height:auto;display:block}}</style></head><body><div class="image-grid">{img_tags}</div></body></html>"""
                self.send_response(200)
            ...
            try:
                ...
                headers = {'User-Agent': spider.headers['User-Agent'], 'Referer': 'https://kemono.cr/'} if spider else {}
                with requests.get(target, headers=headers, proxies=proxies, stream=True, verify=False, timeout=15) as resp:
...
class Spider(Spider):
    ...
    def init(self, extend=""):
        try:
            ...
            self.proxies = {'http': 'http://127.0.0.1:7890', 'https': 'http://127.0.0.1:7890'}#pg，真心修改端口即可
    ...
    def fetch(self, url, headers=None, is_api=False):
        try:
            ...
            return self.session.get(url, headers=req_headers, timeout=20, verify=False)
        except: return None
    ...
    def _get_creator_list(self, service_id, pg):
        ...
        filtered = sorted([c for c in self.creators_cache if c.get('service') == service_id], key=lambda x: x.get('favorited', 0), reverse=True)
        total = len(filtered)
        start, end = (pg - 1) * limit, pg * limit
        style = {"type": "list", "ratio": "1.1"}
        vlist = [{'vod_id': f"creator###{service_id}###{item['id']}", 'vod_name': item.get('name'), 'vod_pic': self._proxify_url(f"https://img.kemono.cr/icons/{service_id}/{item['id']}"), 'vod_tag': 'folder', 'vod_remarks': f"♥ {item.get('favorited', 0)}", "style": style} for item in filtered[start:end]]
    ...
    def _get_post_list(self, tid, pg):
        ...
        if resp and resp.status_code == 200:
            for post in resp.json():
                cover = next((f"https://img.kemono.cr/thumbnail/data{p}" for p in [post.get('file', {}).get('path')] + [a.get('path') for a in post.get('attachments', [])] if p and p.split('.')[-1].lower() in self.img_exts), "https://kemono.cr/static/kemono-logo.svg")
                has_video = False
                if post.get('videos'):
                ...
                if has_video:
                    vlist.append({
                        'vod_id': f"video###{service}###{user_id}###{post.get('id')}", 
                        'vod_name': post.get('title', 'Untitled'),
                        'vod_pic': self._proxify_url(cover),
                        'vod_remarks': '▶ Video',
                        'style': style
                    })
                else:
                    vlist.append({
                        'vod_id': f"post###{service}###{user_id}###{post.get('id')}", 
                        'vod_name': post.get('title', 'Untitled'),
                        'vod_pic': self._proxify_url(cover),
                        'vod_tag': 'folder', 
                        'vod_remarks': post.get('published', '')[:10],
                        'style': style
                    })
    ...
    def detailContent(self, ids):
        try:
            ...
            if post.get('videos'):
                for v in post['videos']:
                    if v.get('path'):
                        base, path = v.get('server', ''), v.get('path', '')
            ...
            vod = {
                "vod_id": tid,
                "vod_name": post.get('title', 'Untitled'),
                "vod_pic": self._proxify_url(next((f"https://img.kemono.cr/thumbnail/data{post.get('file', {}).get('path')}" for p in [1] if post.get('file', {}).get('path')), "")),
                "type_name": service,
                "vod_year": post.get('published', '')[:4],
                "vod_pubdate": post.get('published', '')[:10],
                "vod_area": "Kemono",
                "vod_remarks": f"♥ {post.get('favorited', 0)}",
                "vod_content": re.sub(r'<[^>]+>', '', post.get('content', '')).strip()[:500],
                "vod_play_from": "KemonoPlayer", 
                "vod_play_url": "#".join(play_list) if play_list else "No Video$http://127.0.0.1/"
            }
            return {"list": [vod]}
        except: return {'list': []}
    ...
    def _handle_image_popup_safely(self, tid):
        def load_images_and_show():
            try:
                ...
                proxied_urls = [self._proxify_url(u) for u in list(dict.fromkeys(image_urls))]
    ...
    def searchContent(self, key, quick, pg=1):
        ...
        pg, limit = int(pg), 20
        results = sorted([c for c in self.creators_cache if key.lower() in c.get('name', '').lower()], key=lambda x: x.get('favorited', 0), reverse=True)
        start, end = (pg - 1) * limit, pg * limit
        style = {"type": "list", "ratio": "1.1"}
            ...
            try:
                ...
                for record in records:
                    try:
                        ...
                        pausedField = recordClass.getDeclaredField("paused")
                        pausedField.setAccessible(True)
                        if not pausedField.getBoolean(record):
                            activityField = recordClass.getDeclaredField("activity")
                            activityField.setAccessible(True)
                            act = activityField.get(record)
                            break
                    except: continue
                if not act: return
                ...
                def show_dialog():
                    try:
                        ...
                        w.loadUrl(f"http://127.0.0.1:{self.server_port}/gallery.html?id={cache_key}")
                        d.setContentView(w)
                        d.show()
                    except: pass
